...
public class ZoneControllerTest extends ParentControllerTest {
  ...
  public void testCreate() {
    ...
    ResponseOptions response = given().body("{\"name\":\"house_floods\",\"state\":true}").expect().statusCode(201)
        .body("name", equalTo("house_floods")).body("state", equalTo(true)).when().post("/zone").andReturn();
  }
  ...
  public void testUpdate() {
    ...
    ResponseOptions response = given().body("{\"name\":\"house_floods\",\"state\":true}").expect().statusCode(201)
        .body("name", equalTo("house_floods")).body("state", equalTo(true)).when().put("/zone").andReturn();
  }
  ...
  public void testCreateFail() {
    System.out.println("create");
    super.mockServerCentral.when(request().withPath("/action").withBody(exact("{\"name\":\"hall\",\"state\":true}")))
        .respond(response().withBody("{\"name\":\"hall\",\"state\":true}").withStatusCode(201));
    VerificationTimes.exactly(1);
    CurrentNodeState.refreshNodeState();// build our registry of nodes

    ResponseOptions response =
        given().body("{\"name\":\"hall\",\"state\":true}").expect().statusCode(403).when().post("/zone").andReturn();
    assertTrue(response.getBody().print().contains("This zone is not allowed to be manually changed."));
  }
