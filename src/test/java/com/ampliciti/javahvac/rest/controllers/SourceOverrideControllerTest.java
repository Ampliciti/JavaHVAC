...
public class SourceOverrideControllerTest extends ParentControllerTest {
  ...
  public void testGetAllOverridesSet() throws Exception {
    ...
    OverrideHolder.setSourceOverride("recirculatorPump", SourceOverride.OVERRIDE_ON);
    ResponseOptions response =
        given().expect().statusCode(200).when().get("/sourceOverride").andReturn();
    String responseBody = response.getBody().asString();
    logger.info(response);
  ...
  public void testGetAllOverridesSetMultiple() throws Exception {
    ...
    OverrideHolder.setSourceOverride("recirculatorPump", SourceOverride.OVERRIDE_ON);
    OverrideHolder.setSourceOverride("housePump", SourceOverride.OVERRIDE_OFF);
    OverrideHolder.setSourceOverride("whatever", SourceOverride.RUN);
    ResponseOptions response =
        given().expect().statusCode(200).when().get("/sourceOverride").andReturn();
    String responseBody = response.getBody().asString();
    logger.info(response);
    ...
    assertEquals(SourceOverride.OVERRIDE_ON.toString(), overridesList.get(1).get("state"));
    assertEquals("housePump", overridesList.get(0).get("name"));
    assertEquals(SourceOverride.OVERRIDE_OFF.toString(), overridesList.get(0).get("state"));
  }
}
