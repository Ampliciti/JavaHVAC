...
public class NodeCommandRESTDao implements NodeCommandDao {
   ...
   */
  @Override
  public boolean sendCommand(String nodeAddress, String name, boolean command) throws NodeConnectionException {
    ...
    try {
      ...
      if (jsonResponse != null && jsonResponse.contains(name) && jsonResponse.contains(Boolean.toString(command))) { // quick/lazy
        return true;
      } else {
        return false;
      }
      ...
      if (restResponse != null) {
        throw new NodeConnectionException("Was able to connect to node at: " + nodeAddress
            + ", however, the response was not in the expected format: " + restResponse.toJSONString(), e);
      } else {
        throw new NodeConnectionException(
            "Was able to connect to node at: " + nodeAddress + ", however, no response was returned.", e);
      }
    }
  }
